...
# This script creates a playable Ninja Fruit game scene
# Run this in Unity's play mode or via Unity's scripting API

Write-Host "===============================================" -ForegroundColor Cyan
Write-Host "  Ninja Fruit - Unity Scene Setup Script" -ForegroundColor Cyan
Write-Host "===============================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "Creating Unity scene setup C# script..." -ForegroundColor Yellow

# Create a C# script that Unity will execute to build the scene
$sceneSetupScript = @'
using UnityEngine;
...
public class NinjaFruitSceneBuilder : MonoBehaviour
{
    ...
    public static void BuildGameplayScene()
    {
        ...
        spawnerObj.transform.position = new Vector3(0, -5, 0);
...
$setupScriptPath = "$assetsPath\Editor\NinjaFruitSceneBuilder.cs"
$editorDir = "$assetsPath\Editor"

# Create Editor directory if it doesn't exist
if (-not (Test-Path $editorDir)) {
    New-Item -ItemType Directory -Path $editorDir -Force | Out-Null
    Write-Host "✓ Created Editor directory" -ForegroundColor Green
}

# Write the script
$sceneSetupScript | Out-File -FilePath $setupScriptPath -Encoding UTF8
Write-Host "✓ Created scene builder script at: $setupScriptPath" -ForegroundColor Green
...
Write-Host "2. Wait for Unity to compile the new scripts" -ForegroundColor White
Write-Host "3. Click: Ninja Fruit > Build Gameplay Scene" -ForegroundColor White
Write-Host "4. Press Play button to test the game!" -ForegroundColor White
Write-Host ""
Write-Host "Game Controls:" -ForegroundColor Yellow
Write-Host "- Click and drag mouse to swipe" -ForegroundColor White
Write-Host "- Slice fruits to gain points" -ForegroundColor White
Write-Host "- Avoid bombs (red penalty)" -ForegroundColor White
Write-Host "- Don't miss 3 fruits or game over!" -ForegroundColor White
Write-Host ""
